...
namespace Ems.Authorization.Users
{
    ...
    public class UserAppService : EmsAppServiceBase, IUserAppService
    {
        ...
        private string GetBestRole(List<string> roleNames, string tenantType)
        {
            var tenantRoleHierarchy = StaticRoleNames.Tenants.RoleHierarchy.Where(r => r.TenantType == tenantType).ToList();
            var usersRoles = StaticRoleNames.Tenants.RoleHierarchy.Where(r => r.TenantType == tenantType && roleNames.Contains(r.RoleName)).ToList();
            var bestRole = usersRoles.OrderByDescending(r => r.Level).FirstOrDefault();

            return bestRole.RoleName;
        }

        private List<string> GetAvailableRoles(string usersRoleName, string tenantType)
        {
            var tenantRoleHierarchy = StaticRoleNames.Tenants.RoleHierarchy.Where(r => r.TenantType == tenantType).ToList();
            var usersRole = tenantRoleHierarchy.Where(r => r.RoleName == usersRoleName).FirstOrDefault();
            var availableRoles = tenantRoleHierarchy.Where(r => r.Level <= usersRole.Level).Select(r => r.RoleName).ToList();
